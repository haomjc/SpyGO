...
def gear_to_pinion_kinematics(offset, SIGMA, hand, EPGalpha):
    ...
    Vpg_g = lambda phiG, phiP, phiGdot, phiPdot: sc.hat(sc.bodyJac_globalPOE(Tpg0, np.c_[twistP, twistG], np.array([phiP, phiG]), np.array([0,0]))@np.array([phiPdot, phiGdot]))
    Vpg_p = lambda phiG, phiP, phiGdot, phiPdot: sc.hat(sc.spatialJac_globalPOE(np.c_[twistP, twistG], np.array([phiP, phiG]), np.array([0,0]))@np.array([phiPdot, phiGdot]))
