...
# def get_data_field_names(member, fields = 'all'): # fileds = 'all', 'common', 'cutter', 'machine'
#     if member.lower() == 'pinion':
#         cutterFieldName = 'pinion_cutter_data'
#         subCutterFieldName = '' #'pinCutCnv'
#         commonFieldName = 'pinion_common_data'
#         subCommonFieldName = '' # 'pin'
#         machineFieldName = 'pinion_machine_settings'
#         subMachineFieldName = '' # 'pinCnv'
#     else:
#         cutterFieldName = 'gear_cutter_data'
#         subCutterFieldName = '' #'pinCutCnv'
#         commonFieldName = 'gear_common_data'
#         subCommonFieldName = '' # 'pin'
#         machineFieldName = 'gear_machine_settings'
#         subMachineFieldName = '' # 'pinCnv'
...
def assign_machine_par(data: DesignData, member, flank):

    if member.lower() == 'pinion':
        machine_settings_field = data.pinion_machine_settings
    else:
        machine_settings_field = data.gear_machine_settings

    if flank.lower() == 'concave':
        machine_settings = machine_settings_field.concave
    else:
        machine_settings = machine_settings_field.convex
    ...
    if member.lower() == 'gear' and data.gear_common_data.gen_type.lower() == 'formate':
        H = data.gear_machine_settings.HORIZONTAL
        V = data.gear_machine_settings.VERTICAL
        q0 = atan(V/H)
        S0 = sqrt(V**2 + H**2)
        q0 = q0*180/pi
        return np.array([
...
def assign_Blank_Par(data: DesignData, member):

    if member.lower() == 'pinion':
        common_settings = data.pinion_common_data
        root_angle = data.pinion_machine_settings.concave.ROOTANGLE
    else:
        common_settings = data.gear_common_data
        root_angle = data.gear_machine_settings.concave.ROOTANGLE
    
    c_mat, s_mat = manage_machine_par(member, data.system_data.hand.lower())
...
def assign_tool_par(data: DesignData, member, flank, stfFlank = None):
    
    alphaFlank = 0
    rho_straight = 2e6
    
    if member.lower() == 'pinion':
        member_cutter_settings = data.pinion_cutter_data
    else:
        member_cutter_settings = data.gear_cutter_data

    if flank.lower() == 'concave':
        cutter_settings = member_cutter_settings.concave
    else:
        cutter_settings = member_cutter_settings.convex
    
    Rp = cutter_settings.POINTRADIUS
    alphap = cutter_settings.BLADEANGLE
    rhof = cutter_settings.EDGERADIUS
    if cutter_settings.TYPE.lower() == 'curved':
        rho = cutter_settings.RHO
    else:
        rho = rho_straight

    if cutter_settings.topremTYPE.lower() == 'blended':
        rhoTop = cutter_settings.topremRADIUS
        stftop = cutter_settings.topremDEPTH
    elif cutter_settings.topremTYPE.lower() == 'straight':
        alphaTop = cutter_settings.topremANGLE
        stftop = cutter_settings.topremDEPTH 
        rhoTop = rho_straight
        ...
        alphaTop = 0
        if cutter_settings.topremANGLE != 0:
    ...
    if cutter_settings.flankremTYPE.lower() == 'blended':
        rhoFlank = cutter_settings.flankremRADIUS
        stfFlank = cutter_settings.flankremDEPTH
        alphaFlank = 0
    elif cutter_settings.flankremTYPE.lower() == 'straight':
        alphaFlank = cutter_settings.flankremANGLE
        stfFlank = cutter_settings.flankremDEPTH     
        rhoFlank = rho_straight
    else: # None (feature disabled)
        rhoFlank = rho # initialize to a reasonable blend radius in case an identification activates the
        if stfFlank == None:
            stfFlank = rho/3
        alphaTop = 0
        if cutter_settings.flankremANGLE != 0:
