...
class Hypoid:
    ...
    def __init__(self, designData, toothData, coneData, nProf = 11, nFace = 16, nFillet = 12):
        ...
        self.conjugateData = copy.deepcopy(template_dictionary)
        # nurbs output
    ...
    def constructHypoid(self, designData = None, toothData = None, coneData = None, gearGenType = 'Generated', EPGalpha = np.zeros((4,1)), inputData = "designData", synchronization_flag = False):
        ...
        if inputData.lower() != "designData".lower():
            ...
            self.designData, triplets_pin_CNV, triplets_pin_CVX = approxToolIdentification_casadi(self.designData, 'Pinion', RHO = 500)
...
# identification functions
    ...
    def buildIdentificationProblem(self, member, flank, x_index, lb, ub, zR, problem_type = 'conjugate', bound_points_tol = None, scaling_bounds = None):
        ...
        # check if base rootline has been defined for the current member and flank
        if not hasattr(self, 'originalRootLine') :
            setattr(self, 'originalRootLine', {'pinion':{'concave':[], 'convex':[]}, 'gear':{'concave':[], 'convex':[]}})
            self.originalRootLine[member][flank] = self.rootLineStruct[member][flank]
            root_constraint = self.originalRootLine[member][flank]
