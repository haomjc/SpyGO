...
def dict_to_file(dictionary, filename, tabstring = '', write = True):
    """
...
def dataclass_print(obj, tabstring = '', indent = 3):
    """
    Recursively print the fields of a dataclass or any nested dataclass-like structure.
    ...
    for field in fields(obj):
        ...
        if is_dataclass(value):  # nested dataclass
            print(f"\n{tabstring}{key}:")
            dataclass_print(value, tabstring + ' ' * indent, indent)
        elif isinstance(value, dict):  # fallback for dicts inside dataclasses
            print(f"\n{tabstring}{key}: [dict]")
            for subkey, subvalue in value.items():
                print(f"{tabstring + ' ' * indent}{subkey}: {subvalue}")
        elif isinstance(value, np.ndarray):
            if value.size > 10:
                print(f"{tabstring}{key}: {type(value).__name__} of shape {value.shape}")
            else:
                print(f"{tabstring}{key}: {value}")
        else:
            print(f"{tabstring}{key}: {value}")
    return

def dataclass_to_file(obj, filename, tabstring = '', indent = 3, write = True):
    ...
    for field in fields(obj):
        ...
        if is_dataclass(value):  # nested dataclass
            output_string += f"\n{tabstring}{key}:\n"
            output_string += dataclass_to_file(value, filename, tabstring + ' ' * indent, indent, write = False)
        elif isinstance(value, dict):  # fallback for dicts inside dataclasses
            output_string += f"\n{tabstring}{key}: [dict]\n"
            for subkey, subvalue in value.items():
                output_string += f"{tabstring + ' ' * indent}{subkey}: {subvalue}\n"
        elif isinstance(value, np.ndarray):
            if value.size > 10:
                output_string += f"{tabstring}{key}: {type(value).__name__} of shape {value.shape}\n"
            else:
                output_string += f"{tabstring}{key}: {value}\n"
        else:
            output_string += f"{tabstring}{key}: {value}\n"
