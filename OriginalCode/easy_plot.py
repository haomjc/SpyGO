...
def is_notebook():
    try:
        ...
        if 'IPKernelApp' in get_ipython().config:
            return True # disabled jupyter visualization
    except:
        return False

# Force external interactive window
# pv.set_jupyter_backend(None)  # disables inline plotting
# pv.start_xvfb()  
...
class surface(go):
    ...
    """
    def __init__(self, fig, X = None, Y = None, Z = None, parent = None,
                  show_edges = False,
                  edge_color = 'black',
                    face_color = 'white',
...
class quiver(go):
    ...
    def __init__(self, fig, x, y, z, nx, ny, nz, color = 'black', line_width = 10, parent = None) -> None:
        super().__init__(fig, parent)
        self.XData = x
        self.YData = y
        if z is None:
        ...
        # lines = np.column_stack((np.full(n_points-1, 2), np.arange(n_points-1), np.arange(1, n_points))).flatten()
        # self.object = fig.app.PolyData(points)
        # self.object['vectors'] = normals
        # self.object.lines = lines
        fig.figure.add_arrows(points,
            normals,
            color = color,
            line_width = line_width,
        )
        self.figure.addPlotObject(self)
        self.points = points # transpose to have the same format as the other objects
