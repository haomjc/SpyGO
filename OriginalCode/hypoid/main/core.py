...
class Hypoid:
    ...
    def from_macro_geometry(self, system_data, tooth_data, cone_data, initial_gear_tool_radius = 1000, initial_pinion_tool_radius = 1000):
        ...
        self.update_basic_data(self.designData)
        return self
    ...
    def sample_surface(self, member, flank, sampling_size = None, extend_tip = False, updateData = True, FW_vec = None, return_output = False, triplet_guess = None):
        ...
        if return_output:
            return p, n, p_root, n_root, z, R, csi_theta_phi
    ...
    def update_basic_data(self, new_data, FW_vec = None, no_sync = False, triplets = None):
        ...
        samples = [
            ("pinion", "concave",  0,  "Generating pinion concave flank",  trip_pin_cnv),
            ("pinion", "convex",  25, "Generating pinion convex flank",   trip_pin_cvx),
            ("gear",   "concave", 50, "Generating gear concave flank",    trip_gear_cnv),
            ("gear",   "convex",  75, "Generating gear convex flank",     trip_gear_cvx)
        ]
        
        waitbar = Waitbar(step=0, title="Initializing Hypoid", text="")

        # Outputs
        results_pts = {}
        results_root = {}

        for member, flank, step, message, guess in samples:

            waitbar.update(step=step, message=message)

            pts, _, pts_root, _, z, R, csi_theta_phi = self.sample_surface(
                member,
                flank,
                triplet_guess=guess,
                FW_vec=FW_vec,
            ...
            results_pts[(member, flank)] = pts
            results_root[(member, flank)] = pts_root

            self.interpolate_triplets_over_zr(member, flank, z, R, csi_theta_phi)
                   
        pin_pts_cnv   = results_pts[("pinion", "concave")]
        pin_pts_cvx   = results_pts[("pinion", "convex")]
        gear_pts_cnv  = results_pts[("gear",   "concave")]
        gear_pts_cvx  = results_pts[("gear",   "convex")]

        pin_proot_cnv = results_root[("pinion", "concave")]
        pin_proot_cvx = results_root[("pinion", "convex")]
        gear_proot_cnv = results_root[("gear",   "concave")]
        gear_proot_cvx = results_root[("gear",   "convex")]
    ...
    def build_casadi_derivatives(self):
    ...
    def interpolate_triplets_over_zr(self, member, flank, z, R, triplets, data_type: Literal['base', 'easeOff', 'conjugate', 'opti'] = 'base'):
        ...
        if data_type.lower() == 'base':
            ...
            target = self.identificationProblemEaseOff.interpTriplets
        elif data_type.lower() in {'conjugate', 'conj'}:
            target = self.identificationProblemConjugate.interpTriplets
        elif data_type.lower() in {'opti', 'optimization'}:
            target = self.identificationProblemOptimization.interpTriplets
        else:
    ...
    def identifyConjugatePinion(self,
        ...
        If things wont work we could still fall back to single flank identification at a time.
        ...
        for gear_flank, pinion_flank in [("convex", "concave"), ("concave", "convex")]:
            ...
            new_settings, residuals, triplets, z, R = evaluate_identification_problem(solver, settings, target_points)
            self.identificationProblemConjugate.designData.update_settings("pinion", pinion_flank, x_index, new_settings)
            self.interpolate_triplets_over_zr('pinion', pinion_flank, z, R, triplets, data_type='conjugate')
    ...
    # ! TO DO
    def plot_pattern(self, member, flank, pattern):
        # plots a pattern on to the tooth surface (axial plane)
        # maybe we ca also implement a pattern representation on to the 3D flank surface (best)
        return
    ###################################################
    # LOG FUNCTIONS
    ###################################################
